...
@section('content_header')
    <h5 style="color: blue;">SubLicence</h5>
@stop
@section('content')
    <?php
    ...
    ?>
        <form action="{{ route('sublicence.index') }}" method="get" accept-charset="utf-8" class="form-horizontal">
            <div class="row">
                ...
                <div class="col-md-2">
                    <a class="btn btn-success unicode" href="{{route('sublicence.create')}}"><i class="fas fa-user-cog" /></i> Add New!</a>
                </div>
            </div>
        </form>
        <br>
             ...
             @if($sublicences->count()>0)
                ...
                <tr class="table-tr" data-url="">
                  ...
                  <form action="{{route('sublicence.destroy',$licence->id)}}" method="POST" onsubmit="return confirm('Do you really want to delete?');">
                    @csrf
                    @method('DELETE')
                    <a class="btn btn-sm btn-primary" href="{{route('sublicence.edit',$licence->id)}}" ><i class="fa fa-fw fa-edit" style="padding-top: 5px;padding-bottom: 5px;padding-left: 2px;padding-right: 5px"/></i></a> 
